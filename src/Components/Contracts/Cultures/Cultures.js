...
const Cultures = () => {
  const [cultures, setCultures] = useState([
    { name: "cultures 1", price: 0 },
    { name: "cultures 2", price: 0 },
    { name: "cultures 3", price: 0 },
    { name: "cultures 4", price: 0 },
    { name: "cultures 5", price: 0 },
    { name: "cultures 6", price: 0 },
  ]);
  const { lightMode } = useContext(ColorContext);

  const handlePriceChange = (index, price) => {
    const updatedCultures = [...cultures];
    updatedCultures[index].price = price;
      ...
      >
        <div className="flex items-center justify-between ">
          ...
          >
            ...
            <input
              type="number"
              className="border p-2 rounded bg-transparent w-14 border-gray-500 "
            />
            <span className={` ${lightMode ? "text-black" : "text-white"}`}>
              Results
            </span>
          ...
          >
            ...
            <input
              type="text"
              className="bg-transparent p-2 border border-gray-500 rounded w-32"
            />
          </div>
        </div>
            ...
            <table className="min-w-full divide-y divide-gray-700 ">
              ...
              >
                {cultures.map((culture, index) => (
                  ...
                  >
                    <td className="py-4 px-6 text-sm ">{culture.name}</td>
                    ...
                    <td className="py-4 px-6 text-sm text-white">
                      <input
                        ...
                        className={`p-2 bg-transparent rounded w-14 border border-gray-500 ${
                          lightMode ? "text-black" : "text-white"
                        }`}
                      />
                    </td>
